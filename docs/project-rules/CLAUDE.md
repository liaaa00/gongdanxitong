...
- 日期：2026-09-15
  错误模式：把合同终止日按开区间比较，导致正好满三年的合同被判定为“不满三年”，试用期上限错误降为2个月。
  影响范围：入职页面日期选择、门户日期限制、后端提交和Excel导入校验。
  预防规则：合同期限边界必须按“终止日包含当天”计算，使用终止日加一天与12/36个月阈值比较，并覆盖恰好1年、恰好3年及前后一天的测试。

- 日期：2026-09-14
  错误模式：门户权限迁移为未配置角色创建仅含门户动作的数组，覆盖默认动作，隐藏业务员主工单导入按钮；上线验收仅使用管理员未发现问题。
  影响范围：北仑业务员和组长入职、离职主工单导入入口。
  预防规则：增量权限迁移必须区分缺省继承与显式覆盖，缺省角色不得写入稀疏数组；权限变更上线须以真实业务员和组长分别验证列表按钮、目标路由和实际模板下载，同时保护只读角色及业务范围。

- 日期：2026-09-14
  错误模式：删除入职期限/月数字段的需求只实现了试用期日期校验，遗漏真实导入模板、门户输入和旧月数依赖，仍宣称完成。
  影响范围：入职新增/编辑、Excel 模板及导入、门户提交与上线验收。
  预防规则：删输入字段必须逐项检查模板配置、实际下载的 Excel 表头/说明/颜色/下拉、表单、映射、派生、校验、迁移和历史读取；用真实下载和提交证据验收，不以页面可打开或测试总数代替需求验证。

- 日期：2026-09-11
  错误模式：监控仅按裸业务响应解析，忽略全局响应拦截器的 code/data 包装；网关只按公共 requestId 关联回执，未覆盖超时后重用编号和 ACK 落盘至日志压缩之间的崩溃窗口。
  影响范围：门户结果已展示却未确认回传、旧轮次结果错误进入重试请求、监控重启后丢失在途请求。
  预防规则：监控必须测试实际 API 响应形状并在真实进度请求后核对证据；转发必须使用内部轮次 ID 严格关联接收与返回，同时保留业务幂等键；日志恢复测试必须覆盖 ACK 后压缩前中断，独立回放请求状态与投递队列。

- 日期：2026-09-10
  错误模式：本机数据库端口拒绝连接、Docker 未启动时，未查看项目启动脚本就停止恢复；忽略了项目使用既有便携版 PostgreSQL。
  影响范围：本机数据库启动、门户权限迁移和真实接口验收；曾把可自行启动的本机服务误报为阻断。
  预防规则：先核对项目启动脚本、pg_ctl 路径、原数据目录、postmaster.opts 和监听端口，再启动原实例；本项目本机实例为 D:/pgsql16portable/data、端口 5433，不能以 Docker 状态判断其是否可恢复，禁止为恢复连接另建或覆盖数据库。
...
- 日期：2026-08-06
  错误模式：曾把“年龄、性别、出生日期不出现在客户 Excel”误解为删除或停用系统字段；随后又只核对数据库字段顺序就宣称系统模板已与参考文件一致，没有比较真实下载文件的说明行、必填、颜色、填写要求、示例、列宽和下拉引用。
  影响范围：入职导入模板、身份证派生字段、系统详情展示、模板必填/标黄口径，以及本地不同数据库端口和旧后端构建造成的验证结论。
  预防规则：模板字段与系统字段必须分别建模；Excel 删列不得推导为系统删字段，身份证派生字段必须保留并验证落库展示。任何“与参考 Excel 一致”的结论必须用当前源码、目标数据库和真实模板服务生成文件，逐项结构化比较可见内容、样式和校验规则，并确认运行实例端口与构建版本；只改 seed/migration 或只查数据库不得宣称完成。

- 日期：2026-08-10
  错误模式：用户已明确“页面参保单位对应字段参保机构名称”，但修复只停留在部分前端别名和理想化测试数据，没有把后端列表/详情响应规范成统一字段，也没有在上线后核对生产真实 JSON 键分布，导致用户仍看到空值。
  影响范围：北仑社保公积金增员/减员列表与详情、省外增减员列表、旧浏览器构建兼容，以及历史减员数据是否具备可回填来源的判断。
  预防规则：字段名称映射修复必须同时验证三层：生产数据实际保存键、后端 API 规范响应、前端列表/详情真实渲染；后端应在业务边界统一输出规范字段，前端别名只作兼容。测试必须覆盖新旧字段同时存在时的优先级和真实分页响应形状；源数据缺失时不得用地区、客户或其他近似字段臆造业务值，必须明确区分“显示链路缺陷”和“历史数据缺失”。

- 日期：2026-08-24
  错误模式：用户要求启动独立客户门户后，先完成了传输和安全协议，仍保留测试文本框，没有优先交付客户可见的展示页面。
  影响范围：客户门户第一轮评审无法直接看到、填写和评价实际服务页面，开发顺序偏离用户的体验优先目标。
  预防规则：新建客户可见门户或页面时，先在独立目录完成可打开、可填写、具备基本交互状态的真实界面，再接入跨服务器链路、服务账号和真实数据；基础链路可保留为后台测试能力，不应取代首轮页面交付。
