...
flowchart TB
    ...
    B --> C[admin 首次登录<br/>admin/admin123]
...
幂等，可重复跑：
- `seed-departments.ts`、`seed-roles.ts`、`seed-users.ts`（`admin / admin123`）
...
后端对 `admin/admin123` 首次登录会**强制跳转**到 `/profile/security`；不改密不能进其它页（`auth.guard` 拦截）。
...
| 风险点 | 必做 | 不做会怎样 |
|--------|------|------------|
| `admin / admin123` | 首登立刻改密（强制跳转） | 暴力破解几分钟内失陷 |
