...
| P1-KNOWN-002 | P0 | 后端编译/Seed | `npm.cmd run build` 失败，当前主要报错集中在 `backend/src/database/seeds/seed-field-permissions.ts`：`ReturnType<DataSource['getRepository']<FieldPermission>>` 写法导致 TS 语法/类型错误，并派生 53 个编译错误 | 在 backend 目录执行 `npm.cmd run build` | 后端编译通过，seed 文件全量类型、禁 any，`npm run seed` 可重跑并写入 11 角色、5 部门、54 字段、4 派发规则、字段权限矩阵、admin/admin123 | 待 backend 修复后复测 |
