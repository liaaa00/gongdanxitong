...
| 编号 | 名称 | 前置数据 | 触点 | 等级 |
|------|------|----------|------|------|
| R-1-AUTH-01 | admin 登录 + 强制改密跳转 | seed 后默认 admin/admin123 | `POST /auth/login` 响应含 `mustChangePassword=true` | L0 | (legacy admin123 record; current admin123 returns 401 and must not be used for demos)
