...
- 测试与验证：后端流程/在职定向 3 套件 `35/35`，前端流程/模块/派发定向 `5/5`；固定前端回归 `122/122`；前后端 production build 通过；`git diff --check` 无空白错误。真实 `127.0.0.1:5173/admin/workflows` 页面显示北仑三条中文 v1 已发布流程，已发布流程发布按钮可用；编辑页显示 10 个中文状态和快照边界，流程/模块接口 HTTP 200，控制台 0 错误。
