...
test('phase-one customer portal exposes the agreed entry points', async () => {
  ...
  assert.match(page, /data-progress-filter="attention"[^>]*>待处理</);
  assert.match(page, /const group=done\?'done':\(attention\?'attention':'processing'\)/);
  assert.match(page, /<span class="status status-'\+row\.status\+'">/);
  assert.match(page, /\.status\.needs_correction \{ color: #a04d0b; background: #fdf0e3; \}/);
  assert.match(page, /\.status\.returned \{ color: #8a3229; background: var\(--danger-soft\); \}/);
  assert.match(page, /\.status\.withdrawn, \.status\.void \{ color: #5d6b76; background: #eef1f4; \}/);
  assert.match(page, /\.mode-card\.active, \.mode-card:active \{ border-color: var\(--portal-primary\)/);
  assert.match(page, /button\.classList\.toggle\('active', selected\); button\.setAttribute\('aria-pressed', String\(selected\)\)/);
  assert.match(page, /resignation-excel-dropzone'\)\.classList\.toggle\('has-file'/);
  assert.match(page, /document\.querySelector\('\.salary-file-dropzone'\)\.classList\.toggle\('has-file'/);
  assert.match(page, /tbody tr:hover \{ background: #f5fafd; \}/);
  assert.match(page, /button\.textContent=confirm\?'提交中…':'上传中…'/);
  assert.match(page, /button\.textContent=buttonLabel/);
  assert.match(page, /showToast\(result\.failureCount\?'导入完成，存在失败记录，请查看明细':'批量导入已受理'\)/);
  assert.match(page, /function bindSubjectSelectOnce\(select, business\) \{/);
  assert.match(page, /if \(select\.dataset\.subjectBound === '1'\) return;/);
