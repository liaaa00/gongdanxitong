...
import { ClockCircleOutlined, SyncOutlined, CheckCircleOutlined, ExclamationCircleOutlined, CloseCircleOutlined, FileTextOutlined } from '@ant-design/icons';
...
const ResignationDetail: React.FC = () => {
  ...
  useEffect(() => {
    if (!id) return;
    setLoading(true);
    Promise.all([getWorkOrder(id), getFields('resignation')])
      .then(([orderData, fieldList]) => {
        setOrder(orderData);
        setFields(fieldList);
      })
      .catch(() => message.error('加载离职详情失败'))
      .finally(() => setLoading(false));
  }, [id]);

  if (loading) return <PageContainer loading />;
  if (!order) return <PageContainer header={{ title: '离职详情' }}><Empty description="工单不存在" /></PageContainer>;
  ...
  return (
    <PageContainer header={{ title: '离职详情', extra: [<Button key="back" onClick={() => navigate('/resignation')}>返回列表</Button>] }}>
      <Space direction="vertical" size="large" style={{ width: '100%' }}>
        <Card>
          <Descriptions column={3} bordered size="small">
            <Descriptions.Item label="工单编号"><Tag color="blue">{order.order_no}</Tag></Descriptions.Item>
            <Descriptions.Item label="订单类型"><Tag>离职</Tag></Descriptions.Item>
        ...
        <Tabs items={[
          { key: 'info', label: '离职信息',
            children: (
              <Card>
                <DynamicForm fields={fields} fieldPermissions={permissions} orderType="resignation" initialValues={order.extra_data} readOnly />
              </Card>
            ),
          },
