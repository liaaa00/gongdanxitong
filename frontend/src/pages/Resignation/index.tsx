...
const ResignationList: React.FC = () => {
  ...
  return (
    <PageContainer header={{ title: '离职工单' }}>
      <ProTable<WorkOrderItem>
        actionRef={actionRef}
        columns={columns}
        request={async (params: PageParams) => {
          const result = await getWorkOrders({ ...params, order_type: 'resignation' });
        ...
        headerTitle="离职工单列表"
        toolBarRender={() => [
          <Button key="new" type="primary" icon={<PlusOutlined />} onClick={() => navigate('/resignation/new')}>新建离职</Button>,
