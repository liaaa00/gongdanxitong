...
const AdminCustomers: React.FC = () => {
  ...
  const handleExpand = async (expanded: boolean, record: CustomerItem) => {
    ...
    if (expanded) {
      if (!branchMap[key]) {
        ...
        } catch { message.error('加载商社列表失败'); }
        finally { setBranchLoading((prev) => ({ ...prev, [key]: false })); }
      }
  ...
  const handleBranchSave = async () => {
    ...
    try {
      ...
      setBranchOpen(false); setBranchEditing(null); branchForm.resetFields();
      // 刷新该客户的商社
              ...
              render: (_: any, br: BranchItem) => (
                <Space size="small">
                  <Button size="small" type="link" icon={<EditOutlined />}
                    onClick={() => openBranchEdit(record.id, br)}>编辑</Button>
                  {isAdmin && (
  ...
  const onDel = async (id: string) => {
    try { await deleteCustomer(id); message.success('已删除'); load(); }
    catch (e: any) { message.error(e?.message || '删除失败'); }
  };
  ...
  return (
    <PageContainer header={{ title: '客户管理' }} extra={[
      ...
      <Table
        ...
        columns={[
          { title: '客户编号', dataIndex: 'customer_code', width: 150,
      ...
      {/* 客户表单弹窗 */}
      <Modal title={editing ? '编辑客户' : '新建客户'} open={open} onOk={onSave}
      ...
      {/* 商社表单弹窗 */}
      <Modal title={branchEditing ? '编辑商社' : '新增商社'} open={branchOpen} onOk={handleBranchSave}
        onCancel={() => { setBranchOpen(false); setBranchEditing(null); branchForm.resetFields(); }} destroyOnHidden>
