...
const CONDITIONAL_REQUIRED: ConditionalRequired[] = [
  { field: 'renewal_term_type', value: '固定期限', requireFields: ['renewal_end_date'] },
];

const RenewalNew: React.FC = () => {
  ...
  const handleSubmit = async (values: Record<string, unknown>) => {
    console.log('[新建续签工单] onFinish 触发，提交值：', values);
    if (!customerId) {
      message.warning('请先选择客户');
      return;
    }
    ...
    try {
      ...
      modal.success({
        ...
        okText: '查看工单详情',
        onOk: () => navigate('/renewal/' + result.id),
      });
    } catch (err) {
      console.error('[新建续签工单] 提交失败：', err);
      message.error('提交失败');
  ...
  const handleSubmitClick = async () => {
    try {
      console.log('[新建续签工单] 点击提交，开始校验表单');
      await formRef.current?.validateFields();
      formRef.current?.submit();
    } catch (err) {
      console.error('[新建续签工单] 表单校验失败：', err);
      message.error('表单校验未通过，请检查红色提示字段');
    }
  };

  return (
    <PageContainer header={{ title: '新建续签工单' }}>
      <Card>
        <Form layout="inline" style={{ marginBottom: 16 }}>
          <Form.Item label="客户" required tooltip="选择本续签工单所属客户单位">
            <Select
              showSearch
              allowClear
              placeholder="请选择客户单位"
        ...
        <Space>
          ...
          <Button onClick={() => navigate('/renewal')}>返回列表</Button>
        </Space>
      </Card>
    </PageContainer>
  );
