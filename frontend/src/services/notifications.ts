...
export function getNotificationBucket(item: Pick<NotificationItem, 'biz_type' | 'type' | 'title' | 'content'>): NotificationBucketKey {
  ...
  if (raw.includes('order_field_changed') || raw.includes('creator_modified') || raw.includes('completed_modified') || raw.includes('modified_by_creator') || raw.includes('业务员') && raw.includes('修改')) return 'creator_modified';
  if (raw.includes('dispatched_supplemented') || raw.includes('order_supplement_filled') || raw.includes('field_supplement') || raw.includes('field_supplemented') || raw.includes('backend_supplemented') || raw.includes('field_changed') || raw.includes('field_change') || raw.includes('补充')) return 'field_changed';
...
function notificationMatchesBizTypes(item: NotificationItem, bizTypes: string[]): boolean {
  const normalizedTypes = bizTypes.map(normalizeBizType).filter(Boolean);
  if (normalizedTypes.length === 0) return true;
  const raw = `${normalizeBizType(item.biz_type)} ${normalizeBizType(item.type)} ${item.title || ''} ${item.content || ''}`.toLowerCase();
  return normalizedTypes.some((type) => raw.includes(type));
}
...
export async function getNotifications(params: { unread?: boolean; isRead?: boolean; biz_type?: string; priority?: string; includeDispatch?: boolean; current?: number } & PageParams): Promise<PageResult<NotificationItem>> {
  const safeParams = {
    ...params,
  ...
  if (isMockMode) {
    ...
    if (safeParams.biz_type) {
      ...
      list = list.filter((n) => notificationMatchesBizTypes(n, bizTypes));
    }
...
export async function markNotificationsReadByQuery(params: { biz_type?: string; includeDispatch?: boolean }): Promise<{ success: boolean; affected: number; unread_count: number } | void> {
  if (isMockMode) {
