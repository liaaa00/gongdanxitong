...
const mockFields: FieldConfigItem[] = [
  ...
  { id: 'o_probation_other_salary', field_code: 'probation_other_salary', field_name: '试用期其他工资', field_type: 'number', is_required: false, default_required: false, validation_regex: null, validation_msg: null, dropdown_options: null, placeholder: '请输入试用期其他工资', help_text: null, order_type: 'onboarding', source_category: 'customer_filled', sub_ticket_scope: 'contract', collection_group: '基本信息', business_context: ['onboarding'], display_order: 34, is_active: true },
...
export async function getFields(orderType?: string): Promise<FieldConfigItem[]> {
  ...
  try {
    // 后端字段接口是分页接口；导出模板配置需要完整系统字段库，避免只取第一页导致字段缺失。
    const pageSize = 200;
    const first = await request.get('/admin/fields', { params: { orderType, page: 1, pageSize }, silentError: true } as any) as any;
    ...
    for (let page = 2; page <= totalPages; page += 1) {
      const result = await request.get('/admin/fields', { params: { orderType, page, pageSize }, silentError: true } as any) as any;
