...
test('province scope switch persists and opens isolated out-of-province flows', async ({ page }) => {
  ...
  let listRequestUrl = '';
  await page.route('**/api/in-service-orders*', async (route) => {
        ...
        body: JSON.stringify({
          ...
          data: {
            list: [{
              ...
              order_type: 'in_service',
              order_kind: 'out_of_province_increase',
  ...
  await expect(page).toHaveURL(/\/out-of-province\/increase$/);
  await expect(page.getByText('省外增员', { exact: true }).first()).toBeVisible();
  ...
  await expect(page).toHaveURL(/\/out-of-province\/import\?orderType=out_of_province_increase$/);
  await expect(page.getByRole('radio', { name: '省外增员', exact: true })).toBeChecked();

  await page.locator('.ant-segmented-item-label', { hasText: '省外减员' }).click();
  await expect(page).toHaveURL(/\/out-of-province\/import\?orderType=out_of_province_decrease$/);
  await expect(page.getByRole('radio', { name: '省外减员', exact: true })).toBeChecked();
