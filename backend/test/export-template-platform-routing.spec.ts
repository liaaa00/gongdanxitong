...
describe('ExportTemplatesService platform routing', () => {
  it('splits mixed contract batch exports by electronic-sign platform and uses matching templates', async () => {
    ...
    const upload = {
      saveBuffer: jest.fn(async ({ buffer }: { buffer: Buffer }) => {
        captured = buffer;
        return { fileId: 'file-1', originalName: 'export.xlsx' };
      }),
    };
    ...
    const sheetNames = workbook.worksheets.map((sheet: { name: string }) => sheet.name);
    expect(sheetNames).toContain('\u52b3\u52a8\u5408\u540c\u6279\u5bfc\u5165\u6a21\u677f2026-05-26');
    expect(sheetNames).toContain('Sheet1');
    const suchuangSheet = workbook.getWorksheet('\u52b3\u52a8\u5408\u540c\u6279\u5bfc\u5165\u6a21\u677f2026-05-26');
    const esignSheet = workbook.getWorksheet('Sheet1');
    expect(suchuangSheet.getCell(2, 3).value).toBe('\u8eab\u4efd\u8bc1\u53f7');
    expect(suchuangSheet.getCell(4, 3).value).toBe('ID-do-suchuang');
    expect(suchuangSheet.getCell(4, 5).value).toBe('\u5f20\u4e09');
    expect(esignSheet.getCell(2, 4).value).toBe('\u8bc1\u4ef6\u53f7(\u9009\u586b)-\u7b7e\u7f72\u65b92');
    expect(esignSheet.getCell(5, 1).value).toBe('\u674e\u56db');
    expect(esignSheet.getCell(5, 4).value).toBe('ID-do-esign');
    expect(esignSheet.getRow(3).hidden).toBe(true);
  }, 30000);
