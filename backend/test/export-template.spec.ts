...
describe('ExportTemplatesService', () => {
  ...
  it('exports e-sign contract data into the provided standard workbook template', async () => {
    const template = { id: 'tpl-1', templateName: EXPORT_NAME, moduleCode: 'contract', signPlatform: 'E\u7b7e\u5b9d', fieldList: [
      ...
      { fieldCode: 'contract_template', order: 31 },
    ] };
    const orders = [{ moduleCode: 'contract', handler: null, handlerId: null, status: 'pending', parentOrder: { orderNo: 'ON1', employeeName: '\u5f20\u4e09', employeeIdCard: '3301', extraData: { mobile: '13800000000', id_card_no: '110101199001011234', id_card_type: '\u4e2d\u56fd\u5c45\u6c11\u8eab\u4efd\u8bc1', contract_subject: '\u4e0a\u6d77\u5916\u670d', company_address: '\u4e0a\u6d77', contract_template: '\u6807\u51c6\u6a21\u677f' } } }];
    const { service, upload } = serviceWith(template, orders);
    ...
    expect(sheet.getCell(firstDataRow, 31).value).toBe('\u6807\u51c6\u6a21\u677f');
  });

  it('exports Suchuang contract data into the provided standard workbook template with appended contract columns', async () => {
    const template = { id: 'tpl-1', templateName: EXPORT_NAME, moduleCode: 'contract', signPlatform: '\u901f\u521b', fieldList: [
      ...
      { const: '1.\u65b0\u7b7e', order: 6 },
      { const: '', order: 7 },
      { fieldCode: 'contract_start_date', order: 8 },
      { fieldCode: 'contract_end_date', order: 9 },
      { const: '013.2021\u7248', order: 10 },
      { fieldCode: 'probation_start_date', order: 11 },
      { fieldCode: 'probation_end_date', order: 12, numFmt: 'yyyy/mm/dd', formula: 'IFERROR(EDATE({probation_start_date},VALUE({probation_months}))-1,"")' },
      { fieldCode: 'contract_template', order: 38 },
      { fieldCode: 'contract_subject', order: 39 },
    ] };
    const orders = [{ moduleCode: 'contract', handler: null, handlerId: null, status: 'pending', parentOrder: { orderNo: 'ON1', employeeName: '\u5f20\u4e09', employeeIdCard: '3301', extraData: { customer_code: 'C001', id_card_no: '110101199001011234', need_esign: '\u662f', contract_start_date: '2026-06-01', contract_end_date: '2029-05-31', probation_start_date: '2026-06-01', probation_months: '3', contract_template: '\u6807\u51c6\u6a21\u677f', contract_subject: '\u52b3\u52a8\u5408\u540c\u4e3b\u4f53' } } }];
    const { service, upload } = serviceWith(template, orders);
    ...
    expect(sheet.getCell(firstDataRow, 38).value).toBe('\u6807\u51c6\u6a21\u677f');
    expect(sheet.getCell(firstDataRow, 39).value).toBe('\u52b3\u52a8\u5408\u540c\u4e3b\u4f53');
  });
