...
describe('social insurance state flow guards (0603)', () => {
  ...
  it('routes salesperson field edit after social insurance acceptance into modify approval and keeps withdraw/void approval-bound', async () => {
    ...
    await acceptedContext.service.creatorUpdateFields(ORDER_ID, { fields: { social_base: 7000 }, reason: 'late change' }, creator);
    expect(acceptedOrder.status).toBe(DispatchedOrderStatus.MODIFY_PENDING);
    expect(acceptedOrder.parentOrder.extraData.social_base).toBe(5000);
