...
export async function seedRoles(dataSource: DataSource): Promise<void> {
  ...
  for (const roleSeed of roleSeeds) {
    ...
    if (existed) {
      existed.name = roleSeed.name;
      existed.level = roleSeed.level;
      existed.description = roleSeed.description;
      existed.isActive = roleSeed.isActive;
  ...
  for (const code of deprecatedRoleCodes) {
    const role = await repository.findOne({ where: { code } });
    if (role && role.isActive) {
      role.isActive = false;
      role.name = role.name.includes('（已废弃）') ? role.name : `${role.name}（已废弃）`;
