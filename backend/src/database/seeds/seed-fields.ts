...
interface FieldSeed {
  ...
  /** 是否包含在导入模板中（默认 true，已从模板移除的字段设为 false） */
...
const onboardingCollectionGroups: Record<string, string> = Object.fromEntries([
  ...[
    ...
    'mobile', 'email', 'household_address', 'postal_code',
  ].map((code) => [code, '基本信息']),
  ...[
    ...
    'work_hour_system', 'current_address',
  ...
  ...[
    ...
    'contract_subject', 'company_address', 'project_name', 'work_arrangement', 'contract_template', 'special_contract_template_name',
...
/* =========================================================================
 * 入职（ONBOARDING）系统字段全集。
 *   Excel 输入列由 seed-import-template-fields.ts 单独维护；性别、出生日期、年龄等
 *   系统字段不出现在模板中，但会在导入后由身份证号派生并用于系统展示。
...
const onboardingFields: FieldSeed[] = [
  ...
  { code: 'graduation_school',      name: '毕业院校',     type: FieldType.TEXT,     required: false, defaultRequired: false, helpText: '厦门社保增员时由客户填写或入职联系补充，学历材料通过附件上传。', orderType: ONBOARDING, businessContext: [ONBOARDING], isIncludedInTemplate: false },
  { code: 'major',                  name: '专业',         type: FieldType.TEXT,     required: false, defaultRequired: false, helpText: '厦门社保增员时由客户填写或入职联系补充。', orderType: ONBOARDING, businessContext: [ONBOARDING], isIncludedInTemplate: false },
  { code: 'graduation_date',        name: '毕业时间',     type: FieldType.DATE,     required: false, defaultRequired: false, helpText: '厦门社保增员时由客户填写或入职联系补充，标准格式：年-月-日。', orderType: ONBOARDING, businessContext: [ONBOARDING], isIncludedInTemplate: false },
  ...
  { code: 'household_address',      name: '户籍地址',     type: FieldType.TEXT,     required: false, defaultRequired: false, helpText: '格式：X省X市X区X路X号X室。', orderType: ONBOARDING, businessContext: [ONBOARDING, ...OUT_OF_PROVINCE_BIZ] },
  { code: 'postal_code',            name: '邮编',         type: FieldType.TEXT,     required: false, defaultRequired: false, orderType: ONBOARDING, businessContext: [ONBOARDING], isIncludedInTemplate: false },
  ...
  { code: 'contract_end_date',      name: '合同终止日期', type: FieldType.DATE,     required: false, defaultRequired: false, helpText: '固定期限时必填。标准格式：年-月-日。', conditionalRequired: conditionNotEq('contract_term_type', '无固定期限'), orderType: ONBOARDING, businessContext: [ONBOARDING, ...OUT_OF_PROVINCE_BIZ] },
  { code: 'probation_start_date',   name: '试用期开始日期', type: FieldType.DATE,   required: false, defaultRequired: false, helpText: '非必填，可单独填写试用期开始日期。标准格式：年-月-日。', orderType: ONBOARDING, businessContext: [ONBOARDING] },
  { code: 'probation_months',       name: '试用期（月）', type: FieldType.TEXT,     required: false, defaultRequired: false, helpText: '格式为整数。', orderType: ONBOARDING, businessContext: [ONBOARDING] },
  { code: 'probation_end_date',     name: '试用期结束日期', type: FieldType.DATE,   required: false, defaultRequired: false, helpText: '非必填；填写后按劳动合同期限校验。标准格式：年-月-日。', orderType: ONBOARDING, businessContext: [ONBOARDING] },
  ...
  { code: 'current_address',        name: '现住址（文书送达地址）', type: FieldType.TEXT, required: false, defaultRequired: false, helpText: '入职材料不需要集约收集时必填。格式：X省X市X区X路X号X室。提示：如不由外服联系员工收集该信息，则需在本次收集页面填写', conditionalRequired: conditionEq('need_onboarding_contact', '否'), orderType: ONBOARDING, businessContext: [ONBOARDING] },
  ...
  { code: 'probation_salary',       name: '试用期工资',   type: FieldType.TEXT,     required: false, defaultRequired: false, helpText: '可填写数字、货币格式或文字说明。', orderType: ONBOARDING, businessContext: [ONBOARDING] },
  ...
  { code: 'social_location',        name: '缴纳地',       type: FieldType.DROPDOWN, required: true,  defaultRequired: true, placeholder: '请选择缴纳地', helpText: '选项来自启用劳动合同主体的城市配置。', orderType: ONBOARDING, businessContext: [ONBOARDING] },
  ...
  { code: 'fund_ratio',             name: '公积金比例',   type: FieldType.DROPDOWN, required: false, defaultRequired: false, placeholder: '请选择公积金比例', helpText: '根据缴纳地自动加载可选比例。广州、深圳的单位与个人比例可分别选择。', orderType: ONBOARDING, businessContext: [ONBOARDING] },
  { code: 'supplementary_fund_ratio', name: '补充公积金比例', type: FieldType.DROPDOWN, required: false, defaultRequired: false, placeholder: '请选择补充公积金比例', helpText: '仅当前缴纳地配置了补充公积金比例时显示并可选。', orderType: ONBOARDING, businessContext: [ONBOARDING, RESIGNATION], isIncludedInTemplate: true },
  ...
  { code: 'company_address',        name: '劳动合同主体注册地', type: FieldType.TEXT, required: false, defaultRequired: false, helpText: '由劳动合同主体自动带出；电子签平台为E签宝时必填。', conditionalRequired: conditionEq('esign_platform', 'E签宝'), orderType: ONBOARDING, businessContext: [ONBOARDING] },
  ...
  { code: 'contract_template',      name: '劳动合同模板（标准模板/特殊模板）', type: FieldType.TEXT, required: false, defaultRequired: false, helpText: '选择特殊模板时填写特殊合同模板名称。', conditionalRequired: conditionEq('need_company_contract', '是'), orderType: ONBOARDING, businessContext: [ONBOARDING] },
  { code: 'special_contract_template_name', name: '特殊合同模板名称', type: FieldType.TEXT, required: false, defaultRequired: false, helpText: '选择特殊模板时填写具体模板名称。', conditionalRequired: conditionAnd(conditionEq('need_company_contract', '是'), conditionEq('contract_template', '特殊模板')), orderType: ONBOARDING, businessContext: [ONBOARDING] },
  ...
  { code: 'feedback_deadline',      name: '反馈截止日期', type: FieldType.DATE,     required: false, defaultRequired: false, helpText: '标准格式：年-月-日。', orderType: ONBOARDING, businessContext: [ONBOARDING, RESIGNATION] },
  { code: 'is_common_template',     name: '是否使用通用材料', type: FieldType.DROPDOWN, required: false, defaultRequired: false, options: ['是', '否'], conditionalRequired: conditionEq('need_onboarding_contact', '是'), orderType: ONBOARDING, businessContext: [ONBOARDING, RESIGNATION] },
  { code: 'template_name',          name: '特殊材料收集内容', type: FieldType.TEXT,     required: false, defaultRequired: false, helpText: '选择否时填写额外需要收集的材料，例如学生证、毕业证。', conditionalRequired: conditionAnd(conditionEq('need_onboarding_contact', '是'), conditionEq('is_common_template', '否')), orderType: ONBOARDING, businessContext: [ONBOARDING, RESIGNATION] },
  ...
  { code: 'payroll_location',       name: '发薪地',       type: FieldType.DROPDOWN, required: false, defaultRequired: false, options: [...PAYROLL_LOCATIONS], helpText: '沿用薪酬银行卡模板的67项发薪地，包含北仑总发薪，独立选择，不与合同主体联动。', conditionalRequired: conditionEq('need_company_payroll', '是'), orderType: ONBOARDING, businessContext: [ONBOARDING] },
...
const resignationFields: FieldSeed[] = [
  ...
  { code: 'resignation_cert_tracking_number', name: '离职证明快递单号', type: FieldType.TEXT, required: false, defaultRequired: false, orderType: RESIGNATION, businessContext: [RESIGNATION] },
