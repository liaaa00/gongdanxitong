...
const onboardingFields: FieldSeed[] = [
  ...
  { code: 'email',                  name: '电子邮件',     type: FieldType.EMAIL,    required: false, defaultRequired: false, regex: '^[^@\\s]+@[^@\\s]+\\.[^@\\s]+$', msg: '邮箱格式不正确', orderType: ONBOARDING, businessContext: ON_RENEWAL },
  ...
  { code: 'work_hour_system',       name: '工时制',       type: FieldType.DROPDOWN, required: true,  defaultRequired: true,  options: ['标准工时制'], orderType: ONBOARDING, businessContext: [ONBOARDING] },
  ...
  { code: 'probation_salary',       name: '试用期工资',   type: FieldType.NUMBER,   required: false, defaultRequired: false, orderType: ONBOARDING, businessContext: [ONBOARDING] },
  { code: 'probation_other_salary', name: '试用期其他工资', type: FieldType.NUMBER, required: false, defaultRequired: false, orderType: ONBOARDING, businessContext: [ONBOARDING] },
  ...
  { code: 'need_esign',             name: '是否电子签',   type: FieldType.DROPDOWN, required: false, defaultRequired: false, options: ['是', '否'], conditionalRequired: { op: 'EQ', field: 'need_company_contract', value: '是' }, orderType: ONBOARDING, businessContext: [ONBOARDING] },
  { code: 'esign_platform',         name: '电子签平台',   type: FieldType.DROPDOWN, required: false, defaultRequired: false, options: ['速创', 'E签宝'], conditionalRequired: { op: 'EQ', field: 'need_company_contract', value: '是' }, orderType: ONBOARDING, businessContext: [ONBOARDING] },
  ...
  { code: 'company_address',        name: '甲方住所',     type: FieldType.TEXT,     required: false, defaultRequired: false, conditionalRequired: { op: 'EQ', field: 'need_company_contract', value: '是' }, orderType: ONBOARDING, businessContext: [ONBOARDING] },
  { code: 'project_name',           name: '项目名称',     type: FieldType.TEXT,     required: false, defaultRequired: false, conditionalRequired: { op: 'EQ', field: 'need_company_contract', value: '是' }, orderType: ONBOARDING, businessContext: [ONBOARDING] },
  { code: 'work_arrangement',       name: '安排或调整工作的情况', type: FieldType.TEXT, required: false, defaultRequired: false, conditionalRequired: { op: 'EQ', field: 'need_company_contract', value: '是' }, orderType: ONBOARDING, businessContext: [ONBOARDING] },
  ...
  { code: 'special_remark',         name: '特殊备注',     type: FieldType.TEXT,     required: false, defaultRequired: false, helpText: '无缝转移注意反馈/不要联系员工', orderType: ONBOARDING, businessContext: [ONBOARDING] },
...
/* =========================================================================
 * 离职减员（RESIGNATION）—— 按「社保公积金减员」导入表 10 列口径
 *   表列：姓名 / 身份证号 / 缴纳地区 / 社保公积金停保月 / 离职原因 / 离职日期 /
 *         离职材料是否需要共享收集 / 需要反馈截止日期 / 是否为通用模板 / 模板名称
 *   其中 employee_name / id_card_no 为公共字段（businessContext 含 RESIGNATION），
 *   feedback_deadline / is_common_template / template_name 由 onboarding 段共享（含 RESIGNATION），
 *   此处仅声明 RESIGNATION 专属新字段。
 * ========================================================================= */
const resignationFields: FieldSeed[] = [
  ...
  { code: 'social_stop_month',     name: '社保公积金停保月', type: FieldType.DROPDOWN, required: true,  defaultRequired: true,  options: ['1月', '2月', '3月', '4月', '5月', '6月', '7月', '8月', '9月', '10月', '11月', '12月'], orderType: RESIGNATION, businessContext: [RESIGNATION] },
  ...
  { code: 'resignation_date',      name: '离职日期',         type: FieldType.DATE,     required: true,  defaultRequired: true,  orderType: RESIGNATION, businessContext: [RESIGNATION] },
...
/** 离职旧字段（v1 口径）——重构为减员表 10 列后停用，避免在离职表单/导入中继续出现 */
const DEACTIVATED_FIELD_CODES: string[] = [
  'resignation_type', 'last_work_date', 'contract_terminate_date', 'handover_person',
  'need_resignation_cert', 'cert_delivery_address', 'resignation_contact_feedback',
  'resignation_cert_status', 'social_handover_done', 'final_salary_settled', 'resignation_remark',
];
...
export async function seedFields(dataSource: DataSource): Promise<void> {
  ...
  if (DEACTIVATED_FIELD_CODES.length > 0) {
    await repository
      .createQueryBuilder()
      .update(FieldConfig)
      .set({ isActive: false })
      .where('field_code IN (:...codes)', { codes: DEACTIVATED_FIELD_CODES })
      .execute();
  }
}
