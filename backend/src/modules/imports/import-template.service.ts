...
const ONBOARDING_IMPORT_TEMPLATE_EXCLUDED_FIELDS = new Set([
  'contract_feedback',
  'onboarding_feedback',
  'data_entry_feedback',
  'contract_template',
...
export class ImportTemplateService {
  ...
  private async loadActiveFields(orderType: OrderType): Promise<FieldConfig[]> {
    ...
    return this.filterImportTemplateFields(orderType, fields);
  }

  private filterImportTemplateFields(orderType: OrderType, fields: FieldConfig[]): FieldConfig[] {
    if (orderType !== OrderType.ONBOARDING) {
      return fields;
    }
    return fields
      .filter((field) => !ONBOARDING_IMPORT_TEMPLATE_EXCLUDED_FIELDS.has(field.fieldCode))
      .map((field) => {
  ...
  private writeHeaderAndMetaRows(sheet: Worksheet, fields: FieldConfig[]): void {
    ...
    fields.forEach((field, index) => {
      ...
      requiredRow.getCell(col).value = field.isRequired ? '必填' : '非必填';
