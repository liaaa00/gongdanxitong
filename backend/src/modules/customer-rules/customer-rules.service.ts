...
const RULE_INPUT_FIELDS = new Set([
  'onboardingDefaults', 'resignationDefaults', 'salaryRules', 'sharedEmailRules',
...
export class CustomerRulesService {
  ...
  private async getAccessibleCustomer(customerId: string, _user: JwtUserPayload): Promise<Customer> {
  ...
  private normalizeSalaryRules(input: Record<string, unknown>): SalaryRules {
    const allowed = new Set(['billingDay', 'reminderEnabled', 'reminderWorkdayOffsets']);
    ...
    return { billingDay, reminderEnabled, reminderWorkdayOffsets: [...SALARY_REMINDER_OFFSETS] };
  }
