...
import {
  evaluateHandlingFeedback,
  getHandlingFeedbackItems,
...
export class DispatchedOrderService {
  ...
  constructor(
    ...
    @Optional()
    @InjectRepository(WorkOrderFieldSyncBatch)
    private readonly fieldSyncBatchRepository?: Repository<WorkOrderFieldSyncBatch>,
    @Optional()
  ...
  async findTeam(
    ...
    if (isExportOnlyDispatchModule(moduleCode)) {
      throw businessException(4224, HttpStatus.BAD_REQUEST, '薪酬银行卡导出清单不提供团队工单视图');
    }
  ...
  async listModuleMembers(moduleCode: string, user: JwtUserPayload): Promise<Array<{
    ...
    if (isExportOnlyDispatchModule(moduleCode)) {
      throw businessException(4224, HttpStatus.BAD_REQUEST, '薪酬银行卡导出清单没有负责人池');
    }
  ...
  async findOne(id: string, user: JwtUserPayload): Promise<DispatchedOrderDetailItem> {
    ...
    this.assertWorkflowOperationSupported(order, '查看详情');
  ...
  async complete(
    ...
    if (nextCompleted) await this.markTodoNotificationsReadForDispatchedOrder(order, user.sub);
  ...
  async batchCompleteSocialInsurance(
    ...
    for (const id of uniqueIds) {
      ...
      await this.dispatchedOrderRepository.save(order);
      await this.checkMainOrderComplete(order.parentOrderId);
      if (evaluation.complete) await this.markTodoNotificationsReadForDispatchedOrder(order, user.sub);
      await this.writeLog('dispatched_order', id, user.sub, evaluation.complete ? 'batch_feedback_complete' : 'batch_feedback_processing', this.snapshot(order), { remark, extraData: extraDataPatch });
      processed += 1;
      if (evaluation.complete) completed += 1;
    }

    return { success: true, processed, completed, skipped };
  }
  ...
  async batchImport(
    ...
    if (moduleCode === DispatchModuleCode.PAYROLL_BANK_CARD) {
      throw businessException(4224, HttpStatus.BAD_REQUEST, '薪酬银行卡导出清单不支持导入');
    }
    if (payload.mode === 'status' && !payload.forceAction && !isHandlingFeedbackModule(moduleCode)) {
    ...
    for (let index = 0; index < payload.rows.length; index += 1) {
      ...
      try {
        ...
        if (payload.mode === 'status' && isHandlingFeedbackModule(moduleCode) && !payload.forceAction) {
          const result = await this.applyHandlingFeedbackByBatchImport(order, row, user);
          details.push({
            rowNumber,
            success: true,
            id: order.id,
  ...
  async creatorUpdateFields(
    ...
    if (this.isAcceptedDispatchedOrder(order)) {
      ...
      await this.writeLog('dispatched_order', id, user.sub, 'creator_modify_request', before, {
        ...
        previousStatus,
        status: order.status,
        syncBatchId: syncBatch?.id ?? null,
        sync_batch_id: syncBatch?.id ?? null,
      });
      return this.findOne(id, user);
    }
  ...
  async getFieldSyncRecords(id: string, user: JwtUserPayload): Promise<FieldSyncSummary> {
    const order = await this.loadDispatchedOrder(id);
    this.assertWorkflowOperationSupported(order, '查看字段同步');
    await this.assertCanRead(order, user);
    return this.buildFieldSyncSummary(order.id);
  }
  ...
  private async urgeLoadedOrder(order: DispatchedOrder, reasonInput: string | undefined, user: JwtUserPayload): Promise<void> {
    this.assertWorkflowOperationSupported(order, '催办');
    if (![DispatchedOrderStatus.PENDING, DispatchedOrderStatus.PROCESSING].includes(order.status)) {
   ...
   * 行为：子单重新流转到原模块对应后道，并恢复接单前派单形态（固定派单保留原处理人，公共池保持为空），
  ...
  async resubmitDispatched(
    ...
    // 接单会把公共池的 handlerId 写成接单人，重提时需从接单前审计快照恢复原派单形态。
    order.handlerId = await this.readOriginalDispatchHandlerId(order.id, order.handlerId);
    order.handler = null;
    order.status = DispatchedOrderStatus.PENDING;
    order.voidAt = null;
    order.completedAt = null;
  ...
  async getTimeline(
    ...
    const order = await this.loadDispatchedOrder(id);
    this.assertWorkflowOperationSupported(order, '查看处理日志');
    await this.assertCanRead(order, user);
  ...
  async getSupplementLogs(id: string, user: JwtUserPayload): Promise<Array<{ fieldCode: string; oldValue: string | null; newValue: string | null; supplementedById: string; supplementedAt: Date }>> {
    const order = await this.loadDispatchedOrder(id);
    this.assertWorkflowOperationSupported(order, '查看补录日志');
    await this.assertCanRead(order, user);
  ...
  async confirmDirtyRead(id: string, user: JwtUserPayload): Promise<{ success: boolean; cleared: number }> {
    const order = await this.loadDispatchedOrder(id);
    this.assertWorkflowOperationSupported(order, '确认字段变更');
    await this.assertCanRead(order, user);
  ...
  async batchReassign(
    ...
    for (const id of payload.ids) {
      ...
      if (isExportOnlyDispatchModule(order.moduleCode)) {
        skipped.push({ id, reason: '薪酬银行卡导出清单不支持改派' });
        continue;
      }
  ...
  async reassign(
    ...
    const order = await this.loadDispatchedOrder(id);
    this.assertWorkflowOperationSupported(order, '改派');
  ...
  private buildCompletionExtraDataPatch(order: DispatchedOrder, input: Record<string, unknown>): Record<string, unknown> {
    if (!isHandlingFeedbackModule(order.moduleCode)) return input;
    const patch = { ...input };
    for (const item of getHandlingFeedbackItems(order.moduleCode)) {
      ...
      if (patch[item.remarkField] !== undefined && patch[item.remarkField] !== null) {
        patch[item.remarkField] = String(patch[item.remarkField]).trim();
      }
    }
    return patch;
  }

  private evaluateOrderCompletion(order: DispatchedOrder, extraDataPatch: Record<string, unknown>): ReturnType<typeof evaluateHandlingFeedback> {
    const mergedExtraData = { ...(order.parentOrder.extraData ?? {}), ...extraDataPatch };
    const evaluation = evaluateHandlingFeedback(order.moduleCode, mergedExtraData);
    if (!evaluation.matched) {
      return {
        matched: false,
        complete: true,
        nextStatus: DispatchedOrderStatus.COMPLETED,
        normalizedFields: {},
        missingFields: [],
      };
    }
    for (const [fieldCode, value] of Object.entries(evaluation.normalizedFields)) {
      extraDataPatch[fieldCode] = value;
    }
    return evaluation;
  }

  private extractHandlingFeedbackFields(order: DispatchedOrder, row: BatchImportDispatchedOrderRowDto): Record<string, unknown> {
    const source: Record<string, unknown> = { ...(row.raw ?? {}), ...(row.fields ?? {}) };
    const fields: Record<string, unknown> = {};
    const aliasMap: Record<string, string[]> = {
    ...
    for (const item of getHandlingFeedbackItems(order.moduleCode)) {
      ...
      if (remarkValue !== null) fields[item.remarkField] = remarkValue;
    }
    return fields;
  }

  private async applyHandlingFeedbackByBatchImport(
    order: DispatchedOrder,
    row: BatchImportDispatchedOrderRowDto,
    user: JwtUserPayload,
  ): Promise<{ action: 'complete' | 'processing'; message: string }> {
    const blockedReason = this.describeTerminalBlockedStatus(order, '批量反馈办理结果');
    if (blockedReason) {
      throw businessException(4201, HttpStatus.CONFLICT, blockedReason);
    }
    this.assertParentAllowsDispatchedHandling(order);
    await this.assertCanHandle(order, user);
    if (![DispatchedOrderStatus.PENDING, DispatchedOrderStatus.PROCESSING].includes(order.status)) {
      throw businessException(4201, HttpStatus.CONFLICT, '当前状态不允许批量反馈办理结果');
    }
  ...
  private async applyUserScope(
    ...
    const isBusinessLeaderUser = hasAnyRole(user.roles, BUSINESS_LEADER_ROLES);
  ...
  private async toDetailItem(
    ...
    const permissions = user
      ? await this.fieldPermissionService.getPermissionsForUser(
        user.sub,
        `dispatched:${order.moduleCode}`,
        moduleBusinessScope,
      )
    ...
    const isSocialInsuranceSpecialistDetail = Boolean(
      user
      && order.moduleCode === DispatchModuleCode.SOCIAL_INSURANCE
      && hasAnyRole(user.roles, ['social_insurance_specialist']),
    );
    const currentSocialPermissionFields = isSocialInsuranceSpecialistDetail
  ...
  private async assertCanHandle(order: DispatchedOrder, user: JwtUserPayload): Promise<void> {
    this.assertWorkflowOperationSupported(order, '办理');
    if (this.isAdmin(user)) return;
    if (order.handlerId === user.sub && this.canReadAssignedModule(user, order.moduleCode)) return;
  ...
  private assertCanCreatorRestoreVoid(order: DispatchedOrder, user: JwtUserPayload): void {
    this.assertWorkflowOperationSupported(order, '撤销作废');
  ...
  private assertCreatorActionAllowed(order: DispatchedOrder, actionLabel: string): void {
    this.assertWorkflowOperationSupported(order, actionLabel);
  ...
  private assertWorkflowOperationSupported(order: DispatchedOrder, actionLabel: string): void {
    if (isExportOnlyDispatchModule(order.moduleCode)) {
      throw businessException(4224, HttpStatus.BAD_REQUEST, `薪酬银行卡导出清单仅支持固定模板导出，不支持${actionLabel}`);
    }
  }
  ...
  private async checkMainOrderComplete(parentOrderId: string): Promise<void> {
    ...
    const workflowChildren = children.filter((child) => !isExportOnlyDispatchModule(child.moduleCode));
    if (workflowChildren.length === 0 || workflowChildren.some((child) => child.status !== DispatchedOrderStatus.COMPLETED)) return;
  ...
  private async assertNoCompletedAffectedChildren(sourceOrder: DispatchedOrder, changedFields: string[]): Promise<void> {
    ...
    const impactedCompletedChild = completedChildren.find((child) => fields.some((fieldCode) => this.childContainsField(child, fieldCode, modulesByField)));
    if (impactedCompletedChild) {
      throw businessException(4201, HttpStatus.CONFLICT, '已完成子工单不允许线上修改，请线下变更');
    }
  }

  private async recordFieldSyncBatch(
    sourceOrder: DispatchedOrder,
    diff: Array<{ field: string; before: unknown; after: unknown }>,
    actorUserId: string,
    options: {
      trigger: 'creator_update_before_accept' | 'creator_modify_request';
      status: 'direct_synced' | 'approval_pending';
      itemStatus: 'synced' | 'approval_pending';
      requiresApproval: boolean;
      reason?: string | null;
    },
  ): Promise<WorkOrderFieldSyncBatch | null> {
    if (!this.fieldSyncBatchRepository || !this.fieldSyncItemRepository || diff.length === 0) return null;
    const changedFields = Array.from(new Set(diff.map((item) => item.field).filter(Boolean)));
    ...
    const batch = await this.fieldSyncBatchRepository.save(this.fieldSyncBatchRepository.create({
      workOrderId: sourceOrder.parentOrderId,
      sourceDispatchedOrderId: sourceOrder.id,
      sourceModuleCode: sourceOrder.moduleCode,
    ...
    const items: WorkOrderFieldSyncItem[] = [];
    for (const child of activeChildren) {
      const affectedFields = changedFields.filter((fieldCode) => this.childContainsField(child, fieldCode, modulesByField));
      for (const fieldCode of affectedFields) {
        const item = diffByField.get(fieldCode);
        items.push(this.fieldSyncItemRepository.create({
          batchId: batch.id,
          workOrderId: sourceOrder.parentOrderId,
          dispatchedOrderId: child.id,
          moduleCode: child.moduleCode,
    ...
    if (items.length > 0) await this.fieldSyncItemRepository.save(items);
    return batch;
  }

  private async finalizeLatestFieldSyncBatch(
    order: DispatchedOrder,
    approved: boolean,
    approverId: string,
    comment: string | null,
  ): Promise<WorkOrderFieldSyncBatch | null> {
    if (!this.fieldSyncBatchRepository || !this.fieldSyncItemRepository) return null;
    const batch = await this.fieldSyncBatchRepository.findOne({
      where: {
        sourceDispatchedOrderId: order.id,
        status: 'approval_pending',
      },
      order: { createdAt: 'DESC' },
    });
    ...
    await this.fieldSyncBatchRepository.save(batch);

    const items = await this.fieldSyncItemRepository.find({ where: { batchId: batch.id } });
    const nextStatus = approved ? 'approved' : 'kept_old';
  ...
  private async buildFieldSyncSummary(dispatchedOrderId: string): Promise<FieldSyncSummary> {
    if (!this.fieldSyncBatchRepository || !this.fieldSyncItemRepository) {
      return { latestBatch: null, latest_batch: null, synced: [], approvalPending: [], approval_pending: [], rejected: [], keptOld: [], kept_old: [] };
    }
    const targetItems = await this.fieldSyncItemRepository.find({ where: { dispatchedOrderId } });
    const batchIds = Array.from(new Set(targetItems.map((item) => item.batchId).filter(Boolean)));
    const sourceBatches = await this.fieldSyncBatchRepository.find({
      where: { sourceDispatchedOrderId: dispatchedOrderId },
      order: { createdAt: 'DESC' },
    });
    const targetBatches = batchIds.length > 0
      ? await this.fieldSyncBatchRepository.find({ where: { id: In(batchIds) }, order: { createdAt: 'DESC' } })
      : [];
    const batchMap = new Map<string, WorkOrderFieldSyncBatch>();
    for (const batch of [...sourceBatches, ...targetBatches]) batchMap.set(batch.id, batch);
    const batches = Array.from(batchMap.values()).sort((left, right) => right.createdAt.getTime() - left.createdAt.getTime());
    if (batches.length === 0) {
      return { latestBatch: null, latest_batch: null, synced: [], approvalPending: [], approval_pending: [], rejected: [], keptOld: [], kept_old: [] };
    }
    const items = await this.fieldSyncItemRepository.find({
      where: { batchId: In(batches.map((batch) => batch.id)) },
      order: { createdAt: 'ASC' },
    });
    const itemsByBatch = new Map<string, WorkOrderFieldSyncItem[]>();
    for (const item of items) {
      const bucket = itemsByBatch.get(item.batchId) ?? [];
      bucket.push(item);
      itemsByBatch.set(item.batchId, bucket);
    }
    const latestBatch = this.toFieldSyncBatchView(batches[0], itemsByBatch.get(batches[0].id) ?? []);
    const itemViews = items.map((item) => this.toFieldSyncItemView(item));
    const approvalPending = itemViews.filter((item) => item.status === 'approval_pending');
    const keptOld = itemViews.filter((item) => item.status === 'kept_old');
    const rejected = itemViews.filter((item) => item.status === 'rejected' || item.status === 'kept_old');
    return {
      latestBatch,
      latest_batch: latestBatch,
      synced: itemViews.filter((item) => item.status === 'synced' || item.status === 'approved'),
      approvalPending,
  ...
  private toFieldSyncBatchView(batch: WorkOrderFieldSyncBatch, items: WorkOrderFieldSyncItem[]): FieldSyncBatchView {
    return {
      id: batch.id,
      workOrderId: batch.workOrderId,
      work_order_id: batch.workOrderId,
      sourceDispatchedOrderId: batch.sourceDispatchedOrderId,
      source_dispatched_order_id: batch.sourceDispatchedOrderId,
      sourceModuleCode: batch.sourceModuleCode,
      source_module_code: batch.sourceModuleCode,
      trigger: batch.trigger,
      status: batch.status,
      changedFields: batch.changedFields ?? [],
      changed_fields: batch.changedFields ?? [],
      ...
      items: items.map((item) => this.toFieldSyncItemView(item)),
    };
  }

  private toFieldSyncItemView(item: WorkOrderFieldSyncItem): FieldSyncItemView {
    return {
      id: item.id,
      batchId: item.batchId,
      batch_id: item.batchId,
      dispatchedOrderId: item.dispatchedOrderId,
      dispatched_order_id: item.dispatchedOrderId,
  ...
  private async readOriginalDispatchHandlerId(id: string, fallback: string | null): Promise<string | null> {
    const log = await this.operationLogRepository.findOne({
      where: {
        entityType: 'dispatched_order',
        entityId: id,
        actionType: In(['accept', 'claim', 'start_silent_sign', 'reassign']),
      },
      order: { createdAt: 'DESC' },
    });
    if (!log) return fallback;

    const beforeData = (log.beforeData ?? {}) as Record<string, unknown>;
    if (['accept', 'claim', 'start_silent_sign'].includes(log.actionType)) {
      const handlerId = beforeData.handlerId ?? beforeData.handler_id;
      return typeof handlerId === 'string' && handlerId.length > 0 ? handlerId : null;
    }

    const afterData = (log.afterData ?? {}) as Record<string, unknown>;
    const handlerId = afterData.newHandlerId ?? afterData.new_handler_id ?? afterData.handlerId ?? afterData.handler_id;
    return typeof handlerId === 'string' && handlerId.length > 0 ? handlerId : fallback;
  }
