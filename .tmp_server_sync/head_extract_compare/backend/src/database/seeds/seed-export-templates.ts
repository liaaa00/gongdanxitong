...
// ── 3B. 劳动合同-速创导出（contract / 速创，39 列，表头=字段名行+填写说明行）──
...
const contractSuchuangColumns: Array<[string, string, Source]> = [
  ...
  ['劳动合同模板', '', { fieldCode: 'contract_template' }],
  ['劳动合同主体', '', { fieldCode: 'contract_subject' }],
];
...
// ── 3C. 劳动合同-e签宝导出（contract / E签宝，31 列，4 行表头 R1/R2/R3/R4 原样逐字复刻，数据第 5 行起）──
...
const contractEsignColumns: Array<[string, string, string, string, Source]> = [
  ...
  [FILE_TITLE, '劳动合同模板（标准模板/特殊模板）', 'contract_template', '', { fieldCode: 'contract_template' }],
];
...
// ── 3D. 社保公积金增员导出（social_insurance，13 列）──────────────────────────
