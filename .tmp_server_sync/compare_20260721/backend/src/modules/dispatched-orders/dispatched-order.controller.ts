...
export class DispatchedOrderController {
  ...
  @Get(':id/field-sync')
  @FieldPermissionScenario('dispatched:auto')
  getFieldSyncRecords(@Param('id') id: string, @CurrentUser() user: JwtUserPayload) {
    return this.dispatchedOrderService.getFieldSyncRecords(assertUuidParam(id, '瀛愬伐鍗曚笉瀛樺湪'), user);
  }
